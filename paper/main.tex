...
\begin{enumerate}
    ...
    \item \textbf{Covariate Shift under Perturbations:} Attention mechanisms over past context histories exhibit high sensitivity when unexpected external force impulses or sensor noise induce distribution shifts, leading to compounding autoregressive errors and policy failure.
\end{enumerate}
...
\begin{figure*}[t]
    ...
    \caption{\textbf{Hierarchical Decision Mamba-Liquid (HDML) Architecture.} HDML establishes a dual-rate processing hierarchy: macro-cognitive sequence modeling ($10-20\text{ Hz}$) via an 8-layer Mamba-3 SSM backbone, coupled with micro-actuation continuous-time motor execution ($100-500\text{ Hz}$) via a Closed-Form Liquid Neural ODE (CfC). Heterogeneous sensory states $s_t$, target returns $\hat{R}_t$, and prior actions $a_{t-1}$ are projected into unified sequence tokens $u_t \in \mathbb{R}^{384}$. Optimal Transport Flow Matching produces multi-step action chunks $\mathbf{a}_{t:t+k}$ filtered dynamically by the CfC network to eliminate mechanical chatter and reject external perturbations.}
...
To evaluate representation transferability, we evaluate the generalist \textbf{HDML Foundation Model} ($d_{\text{model}}=384$, 8 layers, total 12.4M parameters per embodiment). We freeze \textbf{11,989,918 parameters} (95.9\%--97.0\% of the total parameters) of the pre-trained backbone and fine-tune only the lightweight Universal Adapter (376k--516k parameters) for 3 epochs (under 30 seconds) on target robot morphologies.
...
\begin{itemize}
    \item \textbf{Variant A (Mamba + MLP Head):} Discarding the Liquid CfC head increases mechanical jerk from $0.7862$ to $0.9412$ and reduces perturbed reward by 44\%, confirming that continuous-time ODE integration serves as the primary mechanism for continuous disturbance attenuation.
    \item \textbf{Variant B (Transformer + Liquid Head):} Replacing the Mamba-3 backbone with a standard Causal Transformer preserves smoothness but increases inference latency from $12.8\text{ ms}$ to $48.6\text{ ms}$ as context length increases to $T=100$.
\end{itemize}
...
To facilitate open scientific verification and reproducibility, all source code, pre-training scripts, foundation checkpoints (\texttt{hdml\_foundation\_best.pt}, 64MB), and evaluation logs are permanently archived with Digital Object Identifier (DOI) \href{https://doi.org/10.5281/zenodo.22020016}{\textbf{10.5281/zenodo.22020016}} and made publicly accessible under the Apache 2.0 License at:
\begin{center}
\url{https://github.com/KienPC1234/HDML} \quad $\mid$ \quad \href{https://doi.org/10.5281/zenodo.22020016}{DOI: 10.5281/zenodo.22020016}
\end{center}
All experiments were executed on an NVIDIA GeForce RTX 4070 SUPER GPU (12GB VRAM) with CUDA 13.2 / Python 3.11.15. Commands to reproduce all benchmark tables are packaged in the unified CLI: \texttt{python scripts/hdml\_cli.py benchmark-foundation} and \texttt{python scripts/hdml\_cli.py benchmark-baselines}.
